...
// echo '<a href="logout.php"><input type="button" class="btn btn-warning btn-lg" name="logout" value="logout" style="float:right"></a>';
// echo '</div>';
...
<body>
  ...
  <li>
    <div class="portfolio-experiment">
  <a href="importtt.php">
    <span class="text">Import Database</span>
    <span class="line -right"></span>
    <span class="line -top"></span>
    <span class="line -left"></span>
    <span class="line -bottom"></span>
  ...
  <li>
    <div class="portfolio-experiment">
  <a href="exporttt.php">
    <span class="text">Export Database</span>
    <span class="line -right"></span>
    <span class="line -top"></span>
    <span class="line -left"></span>
    <span class="line -bottom"></span>
...
<form action="head_dash.php" method="post">
...
<div class="class1" style="margin-left: 35%;margin-top: 10%;position: fixed;background-color: orange;width:40%;border-radius: 20px;">
                    ...
                    <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" method="post" class="" style="padding:25px;" role="form">
                    <div style="margin-top: 30px;">
                        <label for="inputEmail3">
                            <span style="font-size: 20px;color: red;margin-left: 50px;"><b>MAC Address :</b></span></label>
                        
                            <input type="text" name="mac" style="width:60%;margin-left: 50px;" pattern="^[0-9A-Fa-f]{2}:[0-9A-Fa-f]{2}:[0-9A-Fa-f]{2}:[0-9A-Fa-f]{2}:[0-9A-Fa-f]{2}:[0-9A-Fa-f]{2}$" class="form-control" id="inputEmail3" placeholder="Mac" required>
                       </div>
                       
                        <button class="button button2" name="insert" style="margin-left:50px;margin-top: 30px;">Add MAC Addresses</button>
                        <button class="button button2" name="delete" style="margin-left:50px;margin-top: 30px;">Delete MAC Addresses</button>
                       </form>
                        </div>
                </div>
      ...
      if ($_SERVER["REQUEST_METHOD"] == "POST")
      {
        if(isset($_POST['insert']))
        {
          ...
          $encry=hash('SHA256',$mac);
          $query="INSERT INTO mac VALUES('$encry')";
          //mysql_select_db("usb");
        ...
        if(isset($_POST['delete']))
        {
          ...
          $oc=mysqli_num_rows($r);

          $query="DELETE from mac WHERE mac_add='$encry'";
         // mysql_select_db("usb");
          $result=mysqli_query($connection,$query);
         ...
         // mysql_select_db("usb");
          ...
          {
            echo '<script language="javascript">';
            echo 'alert("Record doesnt exist!")';
            echo '</script>'; 
          }
        }
      }
